...
describe('GitToolingError', () => {
  it('should create git command error', () => {
    const error = GitToolingError.gitCommand('git status', 1, 'fatal: not a git repository')
...
describe('normalizeRelativePath', () => {
  ...
  it('should throw on absolute path', () => {
    expect(() => normalizeRelativePath('/absolute/path')).toThrow(GitToolingError)
  })
...
describe('Git Operations', () => {
  ...
  describe('ensureGitRepository', () => {
    ...
    it('should throw for non-git directory', async () => {
      ...
      try {
        await expect(ensureGitRepository(nonGitDir)).rejects.toThrow(GitToolingError)
      } finally {
...
`
      ...
      const content = await require('fs/promises').readFile(join(tempRepo, 'test.txt'), 'utf-8')
  ...
  describe('restoreToCommit', () => {
    it('should restore working tree to commit', async () => {
      ...
      const content = await require('fs/promises').readFile(join(tempRepo, 'file.txt'), 'utf-8')
  ...
  describe('restoreGhostCommit', () => {
    it('should restore ghost commit state', async () => {
      ...
      const modifiedExists = require('fs').existsSync(join(tempRepo, 'modified.txt'))
