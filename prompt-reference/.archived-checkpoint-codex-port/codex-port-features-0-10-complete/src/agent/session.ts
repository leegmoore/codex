...
export interface ToolUseSessionItem {
  type: "tool_use";
  name: string;
  call_id: string;
  input: unknown;
...
export interface ToolResultSessionItem {
  type: "tool_result";
  call_id: string;
  output: unknown;
}
...
export async function listSessions(): Promise<SessionSummary[]> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // Ignore malformed session files; they should not halt listing.
    }
  }
...
export async function readHistory(
  ...
  for (const line of raw.split("\n")) {
    ...
    try {
      ...
      // Ignore malformed lines; matches Rust behavior of skipping invalid entries.
    }
  }
